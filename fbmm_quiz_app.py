...
def main():
    ...
    def render_question(question_title, answer_image_map, key_prefix, owner_map):
        ...
        selected_answer = st.radio("Select an answer:", options, key=key_prefix)

        for label in options:
            st.markdown(f"**{label}**")
            if answer_image_map[label]:
                st.image(GITHUB_IMAGE_BASE_URL + answer_image_map[label], width=150)
